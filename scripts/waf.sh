...
cmd_stats() {
    ...
    local ip_stats
    ip_stats=$( (echo "$events" | _jq_safe '.transaction | (.request.headers["X-Real-Ip"] // .client_ip)' | sort | uniq -c | sort -rn | head -10) || true)
    ...
    (echo "$events" | _jq_safe '.transaction.messages[]? | (.details.ruleId) + " " + .message' | sort | uniq -c | sort -rn | head -10) || true
    echo ""

    echo -e "${CYAN}=== Top 10 URIs ===${NC}"
    (echo "$events" | _jq_safe '.transaction.request.uri' | sort | uniq -c | sort -rn | head -10) || true
    echo ""
